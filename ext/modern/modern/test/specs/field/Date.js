topSuite("Ext.field.Date", [
    ...
    describe("submit format value", function() {
        it("should use the dateFormat as the default", function() {
            makeField({
                value: new Date(2010, 0, 15)
            });
            expect(field.getSubmitValue().toString()).toBe('01/15/2010');
        });

        it("should use submitFormat for parsing", function() {
            makeField({
                value: new Date(2010, 0, 15),
                submitFormat: 'Y-m-d'
            });
            expect(field.getSubmitValue()).toBe('2010-01-15');
        });
        ...
        it("should return null if the value isn't a valid date", function() {
            ...
            expect(field.getSubmitValue()).toBe('');
        });
    });
