...
Ext.define('Ext.dom.Query', function() {
    ...
    return {
         ...
         * @return {HTMLElement[]} An array of DOM elements (not a NodeList as returned by
         * `querySelectorAll`).
         * @param {String} [type="select"] Either "select" or "simple" for a simple selector match
            ...
            ? function(path, root, type, single) {
                ...
                if (!DQ.isXml(root)) {
                    try {
                         ...
                         */
                        if (root.parentNode && (root.nodeType !== 9) && path.indexOf(',') === -1 && !startIdRe.test(path)) { // eslint-disable-line max-len
        ...
        pseudos: {
            ...
            "nth-child": function(c, a) {
                var r = [],
                    ri = -1,
                    m = nthRe.exec(a == "even" && "2n" || a == "odd" && "2n+1" || !nthRe2.test(a) && "n+" + a || a), // eslint-disable-line max-len
                    f = (m[1] || 1) - 0,
                    l = m[2] - 0,
                    i, n, j, cn, pn;
